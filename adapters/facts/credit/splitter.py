#!/usr/bin/env python3
"""Turning a credit field into people and roles. Moved here from `names/inputs.py`.

WHY IT MOVED. Splitting a credit is this fact, and `inputs.py` also loads the built collections,
which is not. Nine of its twelve functions were the splitter, so the file was a fact and a loader
sharing a name.

The entry point wraps these so the interpunct ruling is applied by default. Call the entry point.
"""
import pathlib
import re
import sys
...
from facts.credit.interpunct import INTERPUNCT, SEVERAL                 # noqa: E402
