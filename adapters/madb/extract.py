...
def main():
    ...
    for sid, s in series.items():
        title_to_series.setdefault(norm(primary(s.get("schema:name", ""))), sid)

    by_series, grouping = defaultdict(list), {}
    for r in vols:
        sid = local_id(r.get("schema:isPartOf", ""))
        if sid in series:
            key, how = sid, "series-link"
        else:
            t = norm(primary(r.get("schema:name", "")))
            if t in title_to_series:
                key, how = title_to_series[t], "title-match"
            else:
                key, how = "T:" + t, "title-only"
    ...
    for key, vs in by_series.items():
        ...
        for r in sorted(vs, key=lambda r: r["schema:identifier"]):
            ...
            k = isbn or (flat(r.get("schema:volumeNumber", "")), flat(r.get("schema:datePublished", "")))
            if k in seen:
                continue
            seen.add(k)
            keep.append(r)
    ...
    for sid, vs in sorted(by_series.items()):
        ...
        L = [
            ...
            "source: madb",
            f"source_version: {yaml_str(a.tag)}",
            f"retrieved: {a.retrieved}",
            f"work_id: {wid}",
            f"grouping: {grouping[sid]}",
        ]
        if not synthetic:
            L += [f"madb_id: {sid}",
    ...
    print(f"works written   : {len(by_series)}  -> {out}")
