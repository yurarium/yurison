...
# A health floor for the index. metadata101 held 346,797 dated ISBNs at release 1.2.18, and a run
...
def pair(record):
    ...
    """
    isbn = re.sub(r"[^0-9Xx]", "", extract.flat(record.get("schema:isbn", "")))
    date = extract.flat(record.get("schema:datePublished", "")).strip()
    if not isbn or not _ISO.match(date):
        return None
    return isbn, date
