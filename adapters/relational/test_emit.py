...
def _unit(s):
    """What holds with no build on disk: the emitters over a store made here."""
...
def main(s):
    ...
    #
    # §13 STOPPED `build.py` WRITING THESE FILES, which closes the window §6 opened deliberately:
    # for as long as the compiler wrote them, an emitter could be held to what it produced, and
    # that is what proved the store and the compiler agreed. There is nothing left to disagree,
    # because the emitters are now the only producer.
    #
    # SO THE COMPARISON IS A BONUS AND NOT THE PROOF. A developer with an old build still gets it;
    # a clean checkout does not, and the assertions below the parity block are what hold in both
    # cases. Nobody should read a green run here as a parity proof, which is why this says so.
    _unit(s)
    shipped = BUILD / "credits.json"
    if not shipped.exists():
        print("emit: no build to compare against; the parity block did not run")
        return
