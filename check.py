...
def _input_keys():
    ...
    sys.path.insert(0, str(ROOT / "adapters"))
    import filecache
    # TRACKED AND NEWLY WRITTEN BOTH, for the reason `adapters/lint/tree.own_files` states. A file
    # that is not committed yet does not move this digest, so every remembered pass stood while the
    # tree held a module none of them had ever read: the gate answered `all right` locally and the
    # same commit failed on the runner. A new file is exactly when a source-wide rule is at risk.
